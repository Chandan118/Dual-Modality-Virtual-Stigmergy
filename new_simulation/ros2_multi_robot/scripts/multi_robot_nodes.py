...
class SwarmCoordinator(Node):
    ...
    def __init__(self):
        ...
        # Publishers
        self.status_pub = self.create_publisher(Float32MultiArray, "/swarm_status", 10)
        ...
        self.get_logger().info(
            f"Swarm Coordinator started with {self.num_robots} robots"
        )
...
class RobotNode(Node):
    ...
    def __init__(self, robot_id=0):
        ...
        self.get_logger().info(
            f"Robot {robot_id} initialized at ({self.x:.2f}, {self.y:.2f})"
        )
