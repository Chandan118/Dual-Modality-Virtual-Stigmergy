...
try:
    path_data, path_time, sensor_data, sensor_time, trail_data, trail_time = (
        load_matlab_timeseries("simulation_results.mat")
    )
...
fig.suptitle(
    "Dual-Modality Navigation System Analysis", fontsize=16, fontweight="bold", y=0.98
)
...
floor = patches.Rectangle(
    (0, 0), 5, 5, linewidth=1.5, edgecolor="#333333", facecolor="none"
)
ax1.add_patch(floor)
...
# Max speed limit
ax2.axhline(
    y=0.2, color="red", linestyle="--", linewidth=1.5, label="Max Speed 0.2 m/s"
)
...
summary_lines = [
    ...
    (
        f'Target Status: {"REACHED" if dist_to_target < 0.1 else "NOT REACHED"}',
        False,
        10,
    ),
    ("", False, 10),
...
fig.text(
    0.5,
    0.01,
    "Figure: Dual-Modality Navigation System Analysis - 60s Simulation",
    ha="center",
    fontsize=10,
    fontstyle="italic",
    color="#666666",
