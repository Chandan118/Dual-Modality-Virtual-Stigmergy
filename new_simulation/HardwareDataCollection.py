...
class HardwareDataCollection:
    ...
    def run_power_consumption_experiment(
        self, real_data: Optional[Tuple[np.ndarray, np.ndarray, np.ndarray]] = None
    ):
        """
...
STATUS: PHYSICALLY IMPOSSIBLE
    ...
    def _simulate_trajectory(
        ...
        for i in range(1, n_points):
            ...
            heading_noise = np.clip(
                heading_noise, -max_speed * dt / wheel_base, max_speed * dt / wheel_base
            )
    ...
    def _calculate_cross_track_error(
        self, ideal_x, ideal_y, actual_x, actual_y
    ) -> np.ndarray:
    ...
    def _print_trajectory_results(self):
        ...
        print("--------")
        print(
            f"Mean Cross-Track Error: {t.mean_error*100:.2f} cm ({t.mean_error:.4f} m)"
        )
        print(f"Std Deviation:         {t.std_error*100:.2f} cm ({t.std_error:.4f} m)")
        print(f"Maximum Error:         {t.max_error*100:.2f} cm ({t.max_error:.4f} m)")
        print()
        print(
            f"95th Percentile Error: {t.percentile_95*100:.2f} cm ({t.percentile_95:.4f} m) <-- USE THIS VALUE"
        )
        print()
    ...
    def _plot_trajectory(self):
        ...
        ax1.plot(
            t.actual_x, t.actual_y, "r-", linewidth=1, label="Actual Path", alpha=0.7
        )
        ...
        ax2 = axes[0, 1]
        distances = np.cumsum(
            np.sqrt(np.diff(t.actual_x) ** 2 + np.diff(t.actual_y) ** 2)
        )
...
STATUS: PHYSICALLY IMPOSSIBLE
    ...
    def run_mq135_warmup_experiment(
        self, real_data: Optional[Tuple[np.ndarray, np.ndarray]] = None
    ):
        """
    ...
    def _print_mq135_results(self):
        ...
        print("--------")
        print(
            f"Stabilization Time: {m.stabilization_time:.1f} seconds ({m.stabilization_time/60:.1f} minutes)"
        )
        print()
        ...
        if m.stabilization_time > 60:
            ...
            print(
                f"   Recommendation: Use at least {m.stabilization_time * 1.2:.0f} seconds"
            )
        else:
    ...
    def _plot_mq135_warmup(self):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        fig.suptitle(
            "EXPERIMENT 3: MQ-135 Chemical Sensor Warm-Up",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax2.axhline(
            0.001, color="red", linestyle="--", linewidth=2, label="Stability Threshold"
        )
        ...
        ax3 = axes[1, 0]
        v_norm = (
            (m.voltage - np.min(m.voltage))
            / (np.max(m.voltage) - np.min(m.voltage))
            * 100
        )
        ax3.plot(time_min, v_norm, "b-", linewidth=0.8)
        ax3.axhline(95, color="red", linestyle="--", linewidth=2, label="95% Level")
        ax3.axvline(
            m.stabilization_time / 60, color="green", linestyle="--", linewidth=2
        )
...
time of at least {m.stabilization_time * 1.2:.0f} seconds
    ...
    def _plot_pheromone_decay(self):
        ...
        t = np.linspace(0, 60, 100)
        I0 = 255
        I_decay = I0 * np.exp(-p.decay_constant * t) + I0 * p.residual_fraction * (
            1 - np.exp(-t / 10)
        )

        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        fig.suptitle(
            "EXPERIMENT 4: Virtual Pheromone Decay Model",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def _simulate_slam_error(
        ...
        for seg in range(4):
            ...
            ground_truth[start_idx:end_idx, 0] = corners[seg, 0] + t * (
                corners[(seg + 1) % 4, 0] - corners[seg, 0]
            )
            ground_truth[start_idx:end_idx, 1] = corners[seg, 1] + t * (
                corners[(seg + 1) % 4, 1] - corners[seg, 1]
            )
        ...
        # Distance traveled
        distance = np.cumsum(
            [0] + list(np.sqrt(np.diff(gt_x) ** 2 + np.diff(gt_y) ** 2))
        )
        ...
        perpendicular_component = error_magnitude * np.cos(
            2 * np.pi * np.random.rand(num_points)
        )
    ...
    def _plot_slam_rmse(self):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        fig.suptitle(
            "EXPERIMENT 5: Genuine SLAM RMSE Measurement",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax1 = axes[0, 0]
        ax1.plot(
            s.ground_truth_x, s.ground_truth_y, "b-", linewidth=2, label="Ground Truth"
        )
        ax1.plot(
            s.slam_x, s.slam_y, "r-", linewidth=1, label="SLAM Estimate", alpha=0.7
        )
        ax1.set_xlabel("X (m)")
        ax1.set_ylabel("Y (m)")
        ...
        ax2 = axes[0, 1]
        dist = np.cumsum(
            np.sqrt(np.diff(s.ground_truth_x) ** 2 + np.diff(s.ground_truth_y) ** 2)
        )
        ...
        ax3 = axes[1, 0]
        ax3.hist(
            s.errors * 100, bins=30, color="steelblue", edgecolor="black", alpha=0.7
        )
        ax3.axvline(
            s.rmse * 100,
            color="red",
...
Fabricated Value: 8.7 cm
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for i in range(len(s.errors)):
                writer.writerow(
                    [
                        i,
                        f"{s.ground_truth_x[i]:.4f}",
                        f"{s.ground_truth_y[i]:.4f}",
                        f"{s.slam_x[i]:.4f}",
                        f"{s.slam_y[i]:.4f}",
                        f"{s.errors[i]:.4f}",
                        f"{s.errors[i]*100:.2f}",
    ...
    def generate_comprehensive_report(self):
        ...
        if self.power_data:
            ...
            print(
                f"{'1. Power Consumption':<30} {'Mean Power':<20} {f'{self.power_data.mean_power:.3f} W':<20} {status:<15}"
            )
        ...
        if self.trajectory_data:
            status = (
                "VALID"
                if 1.5 <= self.trajectory_data.percentile_95 * 100 <= 5.0
                else "CHECK"
            )
            print(
                f"{'2. Cross-Track Error':<30} {'95th Percentile':<20} {f'{self.trajectory_data.percentile_95*100:.2f} cm':<20} {status:<15}"
            )
        ...
        if self.mq135_data:
            ...
            print(
                f"{'3. MQ-135 Warm-Up':<30} {'Stabilization Time':<20} {f'{self.mq135_data.stabilization_time:.1f} s':<20} {status:<15}"
            )

        # Pheromone
        if self.pheromone_data:
            print(
                f"{'4. Pheromone Decay':<30} {'Decay Constant':<20} {f'{self.pheromone_data.decay_constant:.4f}':<20} {'DOCUMENTED':<15}"
            )
        ...
        if self.slam_data:
            ...
            print(
                f"{'5. SLAM RMSE':<30} {'RMSE':<20} {f'{self.slam_data.rmse*100:.2f} cm':<20} {status:<15}"
            )
    ...
    def plot_comprehensive_results(self):
        ...
        fig.suptitle(
            "FormicaBot V2 Hardware Validation: Comprehensive Results",
            fontsize=16,
            fontweight="bold",
        )
        ...
        if self.power_data:
            ...
            ax.axhline(
                self.power_data.mean_power, color="red", linestyle="--", linewidth=2
            )
        ...
        if self.trajectory_data:
            ax = axes[0, 1]
            ax.hist(
                self.trajectory_data.cross_track_error * 100,
                bins=20,
                color="steelblue",
                edgecolor="black",
                alpha=0.7,
            ...
            ax.axvline(
                self.trajectory_data.percentile_95 * 100,
                color="red",
                linestyle="--",
                linewidth=2,
            )
        ...
        if self.mq135_data:
            ax = axes[0, 2]
            ax.plot(
                self.mq135_data.time / 60, self.mq135_data.voltage, "b-", linewidth=0.8
            )
            ax.axvline(
                self.mq135_data.stabilization_time / 60,
                color="red",
                linestyle="--",
                linewidth=2,
            )
        ...
        if self.slam_data:
            ax = axes[1, 1]
            ax.plot(
                self.slam_data.ground_truth_x,
                self.slam_data.ground_truth_y,
                "b-",
                linewidth=2,
                label="Ground Truth",
            )
            ax.plot(
                self.slam_data.slam_x,
                self.slam_data.slam_y,
                "r-",
                linewidth=1,
                label="SLAM",
                alpha=0.7,
        ...
        summary = ["HARDWARE VALIDATION SUMMARY", "", "=" * 35, ""]
    ...
    def save_all_data(self):
        ...
        summary = {"timestamp": self.timestamp, "experiments": {}}
