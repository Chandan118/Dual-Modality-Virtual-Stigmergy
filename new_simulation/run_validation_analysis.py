...
print(
    "   Mean lateral deviation: {:.3f} cm ({:.4f} m)".format(
        mean_deviation * 100, mean_deviation
    )
)
print("   Std deviation: {:.3f} cm".format(std_deviation * 100))
print("   Median (50th percentile): {:.3f} cm".format(percentile_50 * 100))
print(
    "\n   ✅ 95th percentile: {:.3f} cm ({:.4f} m)".format(
        percentile_95 * 100, percentile_95
    )
)
print(
    "   ❗ Maximum deviation: {:.3f} cm ({:.4f} m)".format(
        max_deviation * 100, max_deviation
    )
)
...
if max_deviation * 100 < caption_threshold:
    ...
    print(
        "      ⚠️  Caption should be corrected to '< {:.1f} cm'".format(
            max_deviation * 100
        )
    )
...
print(
    "{:<10} {:<30} {:<20}".format(
        "[1]", "Power draw reconciliation", "HARDWARE REQUIRED"
    )
)
print(
    "{:<10} {:<30} {:<20}".format("[1b]", "Sensors 2026 relationship", "PAPER REQUIRED")
)
...
fig.suptitle(
    "FormicaBot V2 Hardware Validation: Priority Review Results",
    fontsize=14,
    fontweight="bold",
)
...
ax2 = axes[0, 1]
ax2.hist(
    lateral_deviations * 100, bins=50, edgecolor="black", alpha=0.7, color="steelblue"
)
ax2.axvline(
    x=percentile_95 * 100,
    color="orange",
...
ax3.axhline(
    y=0.669, color="purple", linestyle="--", linewidth=2, label="Paper: 0.669 W"
)
...
table = ax4.table(
    cellText=priority_data[1:],
    colLabels=priority_data[0],
    loc="center",
    cellLoc="center",
    colWidths=[0.3, 0.5],
)
